// Retro Duel — integrated version (menu, AI, visuals, projectiles, knockback, i-frames, restart working)
...
class Fighter {
  ...
  step(dt){
    ...
    // controls or AI
    if (this.stun <= 0){
      if (this.isAI) { /* AI gets handled externally using ai() */ }
      else this.input();
    }
    ...
    if (this.hitstop <= 0){
      this.vy += GRAVITY;
      if (this.onGround) { this.vx *= FRICTION_GROUND; this.vx = clamp(this.vx, -MAX_SPEED_GROUND, MAX_SPEED_GROUND); }
  ...
  draw(){
    ...
    ctx.fillStyle = outline; ctx.fillRect(this.x, this.y, this.w, this.h);
    ...
    // hands
    if (this.facing === 1) ctx.fillRect(this.x+this.w-6, this.y+22, 6,6);
    else ctx.fillRect(this.x, this.y+22, 6,6);
...
function startMatch(mode, difficulty){
  ...
  if (mode === "multi") p2 = new Fighter(W-260, COLORS.p2, { left:"ArrowLeft", right:"ArrowRight", jump:"ArrowUp", sword:"KeyK", fire:"KeyL", dash:"Semicolon" }, false);
  else p2 = new Fighter(W-260, COLORS.p2, {}, true);
  resetRound();
}
...
function step(ts){
  ...
  // timer
  acc += dt;
  if (!roundOver && acc >= 1){ acc -= 1; roundTime = Math.max(0, roundTime - 1); timerEl.textContent = roundTime; if (roundTime===0){ const t = p1.hp === p2.hp ? "Draw!" : (p1.hp > p2.hp ? "Time! Player 1 Wins!" : "Time! Player 2 Wins!"); endRound(t); } }
  ...
  if (!roundOver && p1 && p2){
    ...
    for (const hb of hitboxes){
      ...
      hb.ttl -= dt;
      if (hb.type === "fireball"){ hb.x += hb.vx; hb.y += hb.vy; if (hb.y + hb.h > FLOOR.y || hb.x < 0 || hb.x + hb.w > W) hb.ttl = 0; particles.push(new Particle(hb.x+hb.w/2, hb.y+hb.h/2, rand(-0.6,0.6), rand(-0.6,0.6), 0.12, Math.random()<0.5?COLORS.fire1:COLORS.fire2)); }
      if (hb.ttl <= 0) hb.alive = false;
      const target = hb.owner === p1 ? p2 : p1;
      if (overlap(hb.rect(), target.rect)) { target.takeHit(hb); if (!hb.pierce) hb.alive = false; updateHPBars(); }
    }
...
function drawBackground(){
  const stripes = 10;
  for (let i=0;i<stripes;i++){ const t=i/(stripes-1); const y = t*H*0.9 + 10; ctx.fillStyle = `rgba(62,249,255,${0.05 + 0.05*Math.sin((performance.now()/600)+i)})`; ctx.fillRect(0,y,W,4); }
  ctx.beginPath(); ctx.arc(W/2,160,80,0,Math.PI*2); ctx.fillStyle="#ff3ec9"; ctx.fill();
  ctx.fillStyle="#10173a"; ctx.beginPath(); ctx.moveTo(0,H*0.7); for(let x=0;x<=W;x+=32){ const y=H*0.7 - Math.abs(Math.sin(x*0.01))*40 - (x%64===0?30:0); ctx.lineTo(x,y);} ctx.lineTo(W,H); ctx.lineTo(0,H); ctx.closePath(); ctx.fill();
}
